...
netlist = AscEditor("3x3_neuromemrsitor.asc")          # Creating Netlist from .asc file

# Set default parameters
netlist.set_parameters(x0=0.1)
netlist.set_component_value('Vin', "PULSE(0 1 0 100n 100n 1.2u 5u)")

# Simulation time period to run for 4  mseconds
...
for raw, log in LTC:
    ...
    vin = raw_file.get_trace('V(vg`)')          # Get the trace data of voltage source
    ...
    im = raw_file.get_trace('Ix(Rd2)')         # Get the trace data of memristor current
    ...
    state = raw_file.get_trace('V(x0)')           # Get the trace data of state x
