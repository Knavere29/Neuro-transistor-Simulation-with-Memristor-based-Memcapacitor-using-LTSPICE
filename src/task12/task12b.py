...
# Objective : To find the input setting of 3X3 pseudo-memcapacitor with different parameters (Row-wise) sweeped via python
task_name = "task12b"
run_file_list = []
            ...
            for x1 in [0.1, 0.192, 0.284]:                            # Switching the time period
                for x2 in [0.1, 0.192, 0.284]:                        # Switching the time period
                    for x3 in [0.1, 0.192, 0.284]:                    # Switching the time period
                        ...
                        run_file_list.append("Run_" + str(run_count) + "_Volt_" + str(voltage) + "V_Ton_" + str(t_on) + "u_T_" + str(t_period) + "u_State1_" + str(x1) + "_State2_" + str(x2) + "_State3_" + str(x3)+"_Row")
                        run_netlist_file = "Run_{}_Volt_{}V_Ton_{}u_T_{}u_State1_{}_State2_{}_State3_{}_Row.net".format(run_count, voltage, t_on, t_period, x1, x2, x3)  # File Naming
...
try:
    if os.path.isdir("empty-dir"):
        os.rmdir("result_task12b")
    else:
        shutil.rmtree("result_task12b")
except Exception as e:
    print("result folder doesn't exist Error :",e)

# Create the directory
directory_name = "result_task12b"
try:
...
for raw in run_file_list:
    ...
    pwd = os.getcwd() # present working directory
    file_name = os.path.join(pwd,"result_task12b",str(raw)+".png")
    print(file_name)
